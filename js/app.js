(() => {
    ...
    var header = $(".js-header"), cloneHeader = header.clone();
    cloneHeader.addClass("header--fixed");
    header.before(cloneHeader);
    $(window).scroll((function() {
        if ($(window).scrollTop() > 99) cloneHeader.addClass("header--is-show"); else cloneHeader.removeClass("header--is-show");
    }));
    function ibg() {
    ...
    const script_form = document.getElementById("myForm");
    script_form.addEventListener("submit", formSend);
    async function formSend(e) {
        e.preventDefault();
        let error = script_formValidate(script_form);
        if (0 === error) {
            ...
            script_form.reset();
        } else success.classList.remove("_successful");
    }
